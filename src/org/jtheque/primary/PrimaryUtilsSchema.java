...
public final class PrimaryUtilsSchema extends AbstractSchema {
    ...
    private void createPrimaryDataTables() {
        jdbcTemplate.update("CREATE TABLE " + IDaoTypes.TABLE + " (ID INT IDENTITY PRIMARY KEY, NAME VARCHAR(150) NOT NULL, IMPL VARCHAR(20) NOT NULL, CONSTRAINT UNIQUE_TYPE UNIQUE(NAME, IMPL))");
        jdbcTemplate.update("CREATE TABLE " + IDaoKinds.TABLE + " (ID INT IDENTITY PRIMARY KEY, NAME VARCHAR(150) NOT NULL, IMPL VARCHAR(20) NOT NULL, CONSTRAINT UNIQUE_KIND UNIQUE(NAME, IMPL))");
        jdbcTemplate.update("CREATE TABLE " + IDaoSagas.TABLE + " (ID INT IDENTITY PRIMARY KEY, NAME VARCHAR(150) NOT NULL, IMPL VARCHAR(20) NOT NULL, CONSTRAINT UNIQUE_SAGA UNIQUE(NAME, IMPL))");
        jdbcTemplate.update("CREATE TABLE " + IDaoLendings.TABLE + " (ID INT IDENTITY PRIMARY KEY, DATE INT NOT NULL, THE_PERSON_FK INT NOT NULL, IMPL VARCHAR(20) NOT NULL)");
        ...
        jdbcTemplate.update("CREATE INDEX KINDS_IDX ON " + IDaoKinds.TABLE + "(ID)");
        jdbcTemplate.update("CREATE INDEX SAGAS_IDX ON " + IDaoSagas.TABLE + "(ID)");
        jdbcTemplate.update("CREATE INDEX LENDINGS_IDX ON " + IDaoLendings.TABLE + "(ID)");
        jdbcTemplate.update("CREATE INDEX TYPES_IDX ON " + IDaoTypes.TABLE + "(ID)");
    ...
    private void createGeneralDataTables() {
        jdbcTemplate.update("CREATE TABLE " + IDaoCountries.TABLE + " (ID INT IDENTITY PRIMARY KEY, NAME VARCHAR(150) NOT NULL UNIQUE)");
        jdbcTemplate.update("CREATE TABLE " + IDaoLanguages.TABLE + " (ID INT IDENTITY PRIMARY KEY, NAME VARCHAR(100) NOT NULL UNIQUE)");
        ...
        jdbcTemplate.update("CREATE INDEX LANGUAGES_IDX ON " + IDaoLanguages.TABLE + "(ID)");
        jdbcTemplate.update("CREATE INDEX COUNTRIES_IDX ON " + IDaoCountries.TABLE + "(ID)");
    }
    ...
    private void createReferentialIntegrityConstraints() {
        jdbcTemplate.update("ALTER TABLE " + IDaoPersons.TABLE + " ADD FOREIGN KEY (THE_COUNTRY_FK) REFERENCES  " + IDaoCountries.TABLE + "  (ID) ON UPDATE SET NULL");
    ...
    private void convertBorrowersToPersons() {
        ...
        for (Object[] borrower : borrowers) {
            jdbcTemplate.update(query, borrower[1], borrower[2], borrower[0], borrower[3], IBorrowersService.PERSON_TYPE);
        }
    ...
    public void importDataFromHSQL(Iterable<Insert> inserts) {
        ...
        importer.match("OD_COUNTRY", "INSERT INTO " + IDaoCountries.TABLE + " (ID, NAME) VALUES (?,?,?)", 0, 2);
        importer.match("OD_LANGUAGE", "INSERT INTO " + IDaoLanguages.TABLE + " (ID, NAME) VALUES(?, ?)", 0, 2);
        importer.match("OD_BORROWER", "INSERT INTO " + IDaoPersons.TABLE + " (ID, NAME, FIRST_NAME, EMAIL, THE_COUNTRY_FK, TYPE) VALUES(?,?,?,?,?,?)", IBorrowersService.PERSON_TYPE, 0, 3, 2, 4, 5);
